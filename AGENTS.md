...
│   │   └── profile/               # 프로필 보기/수정 (edit/ 하위 페이지 포함)
│   └── globals.css
├── components/                    # UI 컴포넌트
│   ├── ChatPanel.tsx              # AI 채팅 패널 (홈 Star Map 통합용)
│   ├── DashboardLayout.tsx        # 사이드바 + 메인 레이아웃
│   ├── KnowledgePanel.tsx         # 토픽 상세 패널 (위키 뷰어)
│   ├── Navbar.tsx                 # 상단 네비게이션 바 (서버)
│   ├── NavbarClient.tsx           # 상단 네비게이션 바 (클라이언트)
│   ├── ProfileEditForm.tsx        # 프로필 수정 폼
│   ├── SignOutButton.tsx          # 로그아웃 버튼
│   ├── StarGraph.tsx              # react-force-graph-2d 기반 별자리 그래프
│   ├── StarMapWrapper.tsx         # Star Map + KnowledgePanel 통합 래퍼
...
├── middleware.ts                   # next-intl 기반 locale 라우팅 미들웨어
├── i18n/
│   ├── navigation.ts              # defineRouting + createNavigation (locales: ['en', 'ko'])
│   └── request.ts                 # 서버 locale 설정

tools/
└── tunnel.js                      # SSH 터널 스크립트 (MySQL + Neo4j)
...
- `src/middleware.ts` (next-intl `createMiddleware`)가 locale 감지 및 리다이렉트 처리
...
### 개발 서버 실행
```bash
npm run dev   # tools/tunnel.js를 백그라운드로 실행하고 next dev 시작
```
- `tools/tunnel.js`가 SSH 터널 2개를 생성: MySQL (로컬 3307 → 원격 3306), Neo4j (로컬 7687 → 원격 7687)
...
npm run test:uat  # UAT (node --experimental-strip-types 기반 로그인 테스트)
npm run lint      # ESLint
